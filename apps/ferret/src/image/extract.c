...
static float dw[IMAGE_DIM] = {6.0, 3.0, 1.5, 4.0, 2.0,  1.0,   4.0,
                              2.0, 1.0, 0.2, 0.4, 0.04, 0.007, 0.007};
...
int image_extract_helper(unsigned char* HSV, unsigned char* mask, int width,
                         int height, int nrgn, cass_dataset_t* ds) {
  img_map_t map;
  unsigned char* b;
  int i, j, c, m, r;
  float v;
