...
static inline double Min(double x, double y) { return y > x ? x : y; }
...
int horizontal(const unsigned char* image, int orig_width, int orig_height,
               unsigned char* resize_image, int width) {
  float* contrib;
...
int vertical(const unsigned char* image, int orig_width, int orig_height,
             unsigned char* resize_image, int height) {
  long y;
  float* contrib;
...
unsigned char* resize(unsigned char* image, int orig_width, int orig_height,
...
int image_init(const char* path) { return 0; }

int image_cleanup(void) { return 0; }
...
void rgb2hsv(const unsigned char* rgb, int width, int height,
             unsigned char* hsv) {
  int i;
...
void hsv2rgb(const unsigned char* hsv, int width, int height,
             unsigned char* rgb) {
  int i;
  for (i = 0; i < width * height; i++) {
...
int image_read_rgb(const char* filename, int* width, int* height,
                   unsigned char** data) {
  ...
  orig = (unsigned char*)malloc(cinfo.output_width * cinfo.output_height *
  ...
  rgb = resize(orig, cinfo.output_width, cinfo.output_height, DEFAULT_SIZE,
               DEFAULT_SIZE);
...
int image_read_hsv(const char* filename, int* width, int* height,
                   unsigned char** data) {
  unsigned char* rgb;
  unsigned char* hsv;

  int n = image_read_rgb(filename, width, height, &rgb);
  hsv = (unsigned char*)malloc(DEFAULT_SIZE * DEFAULT_SIZE * CHAN);
  rgb2hsv(rgb, DEFAULT_SIZE, DEFAULT_SIZE, hsv);
}

int image_read_rgb_hsv(const char* filename, int* width, int* height,
                       unsigned char** data_rgb, unsigned char** data_hsv) {
  unsigned char* rgb;
  unsigned char* hsv;
...
int image_write_rgb(const char* filename, int width, int height,
                    unsigned char* data) {
