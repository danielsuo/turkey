...
//
// modified :		Christian Bienia - cbienia@cs.princeton.edu (64-bit
// support for I/O)
//
...
#pragma warning(                                                               \
    disable : 4996) // disable Microsoft compiler deprecation warnings
#pragma warning(disable : 1786) // disable Intel compiler deprecation warnings

//----------------------------- BMP I/O functions -----------------------------

// load BMP into any data type 1 or 3 channels (bitmap will be converted to
// proper number of channels if needed)
template <class T, int C>
bool FlexLoadBMP(const char* file, FlexImage<T, C>& img);

// Save image of any type 1 or 3 channel to a BMP file  (always saves as 24 bit
// image)
template <class T, int C>
...
// Save either a 1 or 3 channel image to a BMP file  (always saves as 24 bit
// image)
template <int C> bool FlexSaveBMP8u(const char* file, FlexImage<Im8u, C>& img) {
  ...
  if (C == 3) // 24-bit image
    for (int i = img.Height() - 1; i >= 0; i--)
      fwrite(&img(0, i), line, 1,
             f); // write each line of image (image is already padded)
  else
