...
//
// modified :		Christian Bienia - cbienia@cs.princeton.edu (Support for
// endianness)
//-----------------------------------------------------------------------------
...
void ReadBmfh(BITMAPFILEHDR* bmfh, FILE* in) {
...
void ReadBmih(BITMAPINFOHDR* bmih, FILE* in) {
  fread(&bmih->biSize, sizeof(bmih->biSize), 1, in);
  fread(&bmih->biWidth, sizeof(bmih->biWidth), 1, in);
  fread(&bmih->biHeight, sizeof(bmih->biHeight), 1, in);
  fread(&bmih->biPlanes, sizeof(bmih->biPlanes), 1, in);
  fread(&bmih->biBitCount, sizeof(bmih->biBitCount), 1, in);
  fread(&bmih->biCompression, sizeof(bmih->biCompression), 1, in);
  fread(&bmih->biSizeImage, sizeof(bmih->biSizeImage), 1, in);
  fread(&bmih->biXPelsPerMeter, sizeof(bmih->biXPelsPerMeter), 1, in);
  fread(&bmih->biYPelsPerMeter, sizeof(bmih->biYPelsPerMeter), 1, in);
  fread(&bmih->biClrUsed, sizeof(bmih->biClrUsed), 1, in);
  fread(&bmih->biClrImportant, sizeof(bmih->biClrImportant), 1, in);
}
...
bool FlexLoadBMPGray(const char* file, FlexImage<Im8u, 1>& img) {
  ...
  if (in == NULL)
    return (false);
  // WARNING: Extra padding in bmfh causes erroneous reading into all but first
  // field of structure
  ReadBmfh(&bmfh, in);
  ConvertBmfh(&bmfh);
...
bool FlexLoadBMPColor(const char* file, FlexImage<Im8u, 3>& img) {
  ...
  // WARNING: Extra padding in bmfh causes erroneous reading into all but first
  // field of structure
  ReadBmfh(&bmfh, in); // read BMP header
  ConvertBmfh(&bmfh);
  if (bmfh.bfType != 19778) // check for valid BMP file
    return (false);
  ReadBmih(&bmih, in); // read info header
  ConvertBmih(&bmih);
