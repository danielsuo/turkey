...
bool TrackingModel::InitCameras(vector<string>& fileNames) {
  ...
  for (int i = 0; i < (int)fileNames.size(); i++)
    if (!mCameras(i).LoadParams(
            fileNames[i].c_str())) // load each camera calibration
      return false;
  return true;
}
...
bool TrackingModel::Initialize(const string& path, int cameras, int layers) {
  ...
  for (int i = 0; i < cameras; i++)
    calibFiles[i] =
        path + "CALIB" + DIR_SEPARATOR + "Camera" + str(i + 1) + ".cal";
  if (!InitCameras(calibFiles))
    return false; // initialize camera calibration parameters
  if (!InitGeometry(path + "BodyShapeParameters.txt"))
    return false; // initialize geometry parameters
  if (!LoadInitialState(path + "InitialPose.txt"))
    return false; // initialize body pose angles and translations
  if (!LoadPoseParameters(path + "PoseParameters.txt"))
    return false; // initialize pose statistics
  GenerateStDevMatrices(layers, mPoses[0].Params(),
                        mStdDevs); // generate annealing rates for particle
                                   // filter using pose parameters
  return true;
}
...
float TrackingModel::LogLikelihood(const vector<float>& v, bool& valid,
  ...
  valid = false;
  if (!pose.Valid(mPoses[0].Params())) // test for a valid pose (reject
                                       // impossible body angles)
    return -1e10;
  body.ComputeGeometry(pose, mBodies[0].Parameters()); // compute 3D model
                                                       // geometry from pose
                                                       // (generate conic
                                                       // cylinders and their
                                                       // transforms)
  if (!body.Valid()) // test for valid geometry (reject poses with intersecting
                     // body parts)
    return -1e10;
  projections.ImageProjection(body, mCameras); // compute projected 2D points
                                               // into each camera image for
                                               // each body part
  float err = measurements.ImageErrorEdge(
      mEdgeMaps, projections); // compute cylinder edge map term
  err += measurements.ImageErrorInside(mFGMaps,
                                       projections); // compute silhouette term
  valid = true;
  return -err;
}
...
inline void GaussianBlur(FlexImage8u& src, FlexImage8u& dst) {
  float k[] = {0.12149085090552f, 0.14203719483447f, 0.15599734045770f,
               ...
               0.12149085090552f};
  FlexImage8u tmp;
  FlexFilterRowV(src, tmp, k,
                 7); // separable gaussian convolution using kernel k
  FlexFilterColumnV(tmp, dst, k, 7);
}

// Calculate gradient magnitude and threshold to binarize
inline FlexImage8u GradientMagThreshold(const FlexImage8u& src,
                                        float threshold) {
  ...
  for (int y = 1; y < src.Height() - 1; y++) // for each pixel
  {
    ...
    for (int x = 1; x < src.Width() - 1; x++) {
      float xg = -0.125f * ph[-1] + 0.125f * ph[1] - 0.250f * p[-1] +
                 0.250f * p[1] - 0.125f * pl[-1] +
                 0.125f * pl[1]; // calc x and y gradients
...
void TrackingModel::CreateEdgeMap(const FlexImage8u& src, FlexImage8u& dst) {
  FlexImage8u gr =
      GradientMagThreshold(src, 16.0f); // calc gradient magnitude and threshold
...
bool TrackingModel::GetObservation(float timeval) {
  ...
  for (int i = 0; i < n; i++) {
    FGfiles[i] = mPath + "FG" + str(i + 1) + DIR_SEPARATOR + "image" +
                 str(frame, 4) + ".bmp";
    ImageFiles[i] = mPath + "CAM" + str(i + 1) + DIR_SEPARATOR + "image" +
                    str(frame, 4) + ".bmp";
  }
  FlexImage8u im;
  for (uint i = 0; i < FGfiles.size(); i++) {
    if (!FlexLoadBMP(FGfiles[i].c_str(),
                     im)) // Load foreground maps and raw images
    {
...
bool TrackingModel::OutputBMP(const std::vector<float>& pose, int frame) {
  ...
  Im8u yellow[3] = {0, 255, 255}, cyan[3] = {255, 255, 0},
       magenta[3] = {255, 0, 255};

  mPoses[0].Set(pose); // set pose angles and translation
  mBodies[0].ComputeGeometry(
      mPoses[0], mBodies[0].Parameters()); // compute 3D model geometry from
                                           // pose (generate conic cylinders and
                                           // their transforms)
  mProjections[0].ImageProjection(mBodies[0], mCameras); // compute projected 2D
                                                         // points into each
                                                         // camera image for
                                                         // each body part
  int levels = int(ImageFiles.size() - 1) / 2 + 1;
  int w = mEdgeMaps[0].Width() / 2, h = mEdgeMaps[0].Height() / 2;
  FlexImage<Im8u, 3> result(w * 2, h * levels),
      dstImage; // create result image and sub-image object

  for (int camera = 0; camera < (int)ImageFiles.size();
       camera++) // create new image for each camera view
  {
    FlexImage<Im8u, 1> srcImage, imds;
    FlexLoadBMP(ImageFiles[camera].c_str(),
                srcImage);           // load raw image from file
    FlexDownSample2(srcImage, imds); // downsample image by factor of 2
    dstImage = result((camera % 2) * w, int(camera / 2) * h, w,
                      h); // create sub-image in result image
    FlexGrayToRGB(
        imds, dstImage,
        false); // convert downsampled image to RGB into sub-image of result
    ProjectedBody& pb = mProjections[0](camera);
    for (int i = 0; i < pb.Size();
         i++) // draw projected cylinders colored by body part
    {
      ...
      if (i == 7 || i == 8 || i == 3 || i == 4)
        ...
        color = magenta;
      FlexLine(dstImage, (int)c.mPts[0].x / 2, (int)c.mPts[0].y / 2,
               (int)c.mPts[1].x / 2, (int)c.mPts[1].y / 2, color);
      FlexLine(dstImage, (int)c.mPts[1].x / 2, (int)c.mPts[1].y / 2,
               (int)c.mPts[2].x / 2, (int)c.mPts[2].y / 2, color);
      FlexLine(dstImage, (int)c.mPts[2].x / 2, (int)c.mPts[2].y / 2,
               (int)c.mPts[3].x / 2, (int)c.mPts[3].y / 2, color);
      FlexLine(dstImage, (int)c.mPts[3].x / 2, (int)c.mPts[3].y / 2,
               (int)c.mPts[0].x / 2, (int)c.mPts[0].y / 2, color);
    }
  }
