...
void AsyncImageLoader::LoadSet(std::vector<std::string>& FGfiles,
                               BinaryImageSet& FGimages,
  ...
  for (unsigned int i = 0; i < FGfiles.size(); i++) {
    ...
    if (!FlexLoadBMP(ImageFiles[i].c_str(), images[i])) {
      std::cout << "Unable to load image: " << ImageFiles[i].c_str()
                << std::endl;
      mFailed = true;
    }
  }
  ...
  while (mCurrentFrame < mNumFrames && !mFailed) {
    for (unsigned int i = 0; i < mNumCameras; i++) // generate filenames
    {
      FGfiles[i] = mPath + "FG" + str(i + 1) + DIR_SEPARATOR + "image" +
                   str(mCurrentFrame, 4) + ".bmp";
      ImageFiles[i] = mPath + "CAM" + str(i + 1) + DIR_SEPARATOR + "image" +
                      str(mCurrentFrame, 4) + ".bmp";
    }

    LoadSet(FGfiles, FGImages, ImageFiles,
            images); // load the data and convert FG images to binary
    ...
    {
      mLock1.Lock();
      mCondFull.Wait(); // wait on full condition (wait until a request removes
                        // a set of images)
      mLock1.Unlock();
    }
    mCurrentFrame++;
...
bool AsyncImageLoader::GetNextImageSet(ImageSet& images,
                                       BinaryImageSet& FGMaps) {
  if (mFailed)
    return false;

  while (mImageBuffer.size() ==
         0) // if buffer is empty, wait until images are loaded
  {
    mLock2.Lock();
    mCondEmpty.Wait();
    mLock2.Unlock();
