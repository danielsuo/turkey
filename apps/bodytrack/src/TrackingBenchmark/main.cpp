...
// Add defines USE_OPENMP, USE_THREADS or USE_TBB for threaded versions if not
...
bool ProcessCmdLine(int argc, char** argv, string& path, int& cameras,
                    int& frames, int& particles, int& layers, int& threads,
                    int& threadModel, bool& OutputBMP) {
  string usage(
      "Usage : Track (Dataset Path) (# of cameras) (# of frames to process)\n");
  usage += string("              (# of particles) (# of annealing layers) \n");
  usage += string("              [thread model] [# of threads] [write .bmp "
                  "output (nonzero = yes)]\n\n");
  usage +=
      string("        Thread model : 0 = Auto-select from available models\n");
  ...
  threadModel = 0;
  if (argc <
      7) // use default single thread mode if no threading arguments present
    return true;
...
#if defined(USE_OPENMP)
int mainOMP(string path, int cameras, int frames, int particles, int layers,
            int threads, bool OutputBMP) {
  cout << "Threading with OpenMP" << endl;
  if (threads < 1) // Set number of threads used by OpenMP
    omp_set_num_threads(
        omp_get_num_procs()); // use number of processors by default
  else
  ...
  ParticleFilterOMP<TrackingModel> pf; // particle filter (OMP threaded)
                                       // instantiated with body tracking model
                                       // type
  pf.SetModel(model);                  // set the particle filter model
  pf.InitializeParticles(particles);   // generate initial set of particles and
                                       // evaluate the log-likelihoods

  cout << "Using dataset : " << path << endl;
  cout << particles << " particles with " << layers << " annealing layers"
       << endl
       << endl;
...
int mainPthreads(string path, int cameras, int frames, int particles,
                 int layers, int threads, bool OutputBMP) {
  cout << "Threading with Posix Threads" << endl;
  if (threads < 1) {
    cout << "Warning: Illegal or unspecified number of threads, using 1 thread"
         << endl;
    threads = 1;
  }
  ...
  ParticleFilterPthread<TrackingModel> pf(
      workers); // particle filter instantiated with body tracking model type
  pf.SetModel(model); // set the particle filter model

  workers.RegisterCmd(workers.THREADS_CMD_PARTICLEWEIGHTS,
                      pf); // register particle filter commands
...
int mainTBB(string path, int cameras, int frames, int particles, int layers,
            int threads, bool OutputBMP) {
  ...
  ParticleFilterTBB<TrackingModelTBB> pf; // particle filter (TBB threaded)
                                          // instantiated with body tracking
                                          // model type
  ...
  // Create the TBB pipeline - 1 stage for image processing, one for particle
  // filter update
...
int mainSingleThread(string path, int cameras, int frames, int particles,
  ...
  model.GetObservation(0); // load data for first frame
  ParticleFilter<TrackingModel>
      pf; // particle filter instantiated with body tracking model type
  pf.SetModel(model);                // set the particle filter model
  pf.InitializeParticles(particles); // generate initial set of particles and
                                     // evaluate the log-likelihoods
...
void print_bytes(void* bytes, int len) {
  unsigned char* bytes_char = (unsigned char*)bytes;
  for (int i = 0; i < len; i++) {
    fprintf(stderr, "%02x ", bytes_char[i]);
  }
  fprintf(stderr, "\n");
}

int main(int argc, char** argv) {
  // FlexImage<Im8u, 1> img;
  // FlexImage<Im8u, 1> tmp;
  // bool ok = FlexLoadBMP8u(argv[1], tmp);
  // char* file = argv[1];
  //
  // FILE* in;
  // BITMAPFILEHDR bmfh;
  ...
  // ReadBmfh(&bmfh, in);
  //
  // print_bytes((void*)&bmfh, BFHSIZE);
  // print_bytes((void*)&bmfh.bfOffBits, sizeof(bmfh.bfOffBits));
  //
  ...
  //   std::cout << "Not a BMP file: " << file << std::endl;
  // }
  //
  ...
  // FlexImage<Im8u, 1> im;
  // if (!FlexLoadBMP(argv[1], im)) // Load foreground maps and raw images
  // {
...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
  cout << "PARSEC Benchmark Suite Version
          "__PARSEC_XSTRING(PARSEC_VERSION) << endl << flush;
#else
  ...
  if (!ProcessCmdLine(argc, argv, path, cameras, frames, particles, layers,
                      threads, threadModel, OutputBMP))
    return 0;
  ...
  switch (threadModel) {
  case 0:
    // This case should never happen, we auto-select the
    // thread model before this switch
  ...
  case 1:
#if defined(USE_TBB)
    mainTBB(path, cameras, frames, particles, layers, threads,
            OutputBMP); // Intel TBB
    threads tracking break;
#else
  ...
  case 3:
#if defined(USE_OPENMP)
    mainOMP(path, cameras, frames, particles, layers, threads,
            OutputBMP); // OpenMP threaded tracking
    break;
#else
    cout << "Not compiled with OpenMP support. " << endl;
    cout << "If the environment supports OpenMP, rebuild with USE_OPENMP "
            "#defined."
         << endl;
    break;
#endif
  ...
  case 4:
    mainSingleThread(path, cameras, frames, particles, layers,
                     OutputBMP); // single threaded tracking
    break;
