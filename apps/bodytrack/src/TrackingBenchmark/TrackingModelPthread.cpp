...
// Define if functions are to be executed with only a single thread (for
// debugging)
//#define SINGLE_THREADED

// constructor
TrackingModelPthread::TrackingModelPthread(WorkPoolPthread& _workers)
    : IOthreadStarted(false), workers(_workers), workInit(_workers.Size()),
      WORKUNIT_SIZE_FILTERROW(8), WORKUNIT_SIZE_FILTERCOLUMN(8),
      WORKUNIT_SIZE_GRADIENT(8){};
...
inline void TrackingModelPthread::GaussianBlurPthread(FlexImage8u* src,
                                                      FlexImage8u* dst) {
...
// Pthreads threaded - 1D filter Row wise 1 channel any type data or kernel
// valid pixels only
template <class T, class T2>
inline void FlexFilterRowVPthread(int y, FlexImage<T, 1>& src,
                                  FlexImage<T, 1>& dst, T2* kernel,
...
// Pthreads threaded - 1D filter Column wise 1 channel any type data or kernel
// valid pixels only
template <class T, class T2>
inline void FlexFilterColumnVPthread(int y, FlexImage<T, 1>& src,
                                     FlexImage<T, 1>& dst, T2* kernel,
                                     int kernelSize, bool allocate = true) {
  int n = kernelSize / 2;
...
inline void GradientMagThresholdPthread(int y, FlexImage8u& src,
                                        float threshold, FlexImage8u& r) {
...
void TrackingModelPthread::Exec(threads::thread_cmd_t cmd,
                                threads::thread_rank_t rank) {
  int i, ticket;
  ...
  if (cmd == workers.THREADS_CMD_FILTERROW) {
    ...
    if (rank == 0) {
      ...
      while (ticket < FilterArgs.src->Height()) {
        for (i = ticket; i < FilterArgs.src->Height() &&
                         i < ticket + WORKUNIT_SIZE_FILTERROW;
             i++) {
          FlexFilterRowVPthread(i, *FilterArgs.src, *FilterArgs.dst,
                                FilterArgs.kernel, FilterArgs.kernelSize,
                                FilterArgs.allocate);
        }
        ticket = loopTickets.getTicket();
      }
    ...
    if (rank == 0) {
      ...
      loopTickets.resetDispenser(FilterArgs.kernelSize / 2,
                                 WORKUNIT_SIZE_FILTERCOLUMN);
    }
    workInit.Wait();
    ...
    if (rank == 0) {
      ...
      while (ticket < FilterArgs.src->Height() - FilterArgs.kernelSize / 2) {
        ...
        for (i = ticket;
             i < FilterArgs.src->Height() - FilterArgs.kernelSize / 2 &&
             i < ticket + WORKUNIT_SIZE_FILTERCOLUMN;
             i++) {
          FlexFilterColumnVPthread(i, *FilterArgs.src, *FilterArgs.dst,
                                   FilterArgs.kernel, FilterArgs.kernelSize,
                                   FilterArgs.allocate);
        }
        ticket = loopTickets.getTicket();
    ...
    if (rank == 0) {
      ...
      while (ticket < GradientArgs.src->Height() - 2) {
        for (i = ticket; i < GradientArgs.src->Height() - 2 &&
                         i < ticket + WORKUNIT_SIZE_GRADIENT;
             i++) {
          GradientMagThresholdPthread(i + 1, *GradientArgs.src,
                                      GradientArgs.threshold, *GradientArgs.gr);
        }
        ticket = loopTickets.getTicket();
      }
#ifdef SINGLE_THREADED
...
bool TrackingModelPthread::GetObservation(float timeval) {
  ...
  if (!imageLoader.GetNextImageSet(images, mFGMaps)) // get next set of images
                                                     // and foreground maps
                                                     // (blocks on empty queue)
    return false;
  for (unsigned int i = 0; i < images.size();
       i++) // create edge maps from images
    CreateEdgeMap(images[i], mEdgeMaps[i]);
  return true;
}

TrackingModelPthread::~TrackingModelPthread() throw() { delete IOthread; }
