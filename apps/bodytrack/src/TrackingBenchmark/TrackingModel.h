...
//				  here.  Memory is allocated for separate
// threads
...
protected:
  std::vector<BinaryImage>
      mFGMaps; // Background segmented images from each camera
  std::vector<FlexImage8u> mEdgeMaps; // edge processed images from each camera
  std::vector<std::vector<float>> mStdDevs; // standard deviations for each
                                            // layer of annealed particle
                                            // filtering
  ...
  std::vector<MultiCameraProjectedBody>
      mProjections; // Image projections of the body on all cameras
  std::vector<ImageMeasurements>
      mImageMeasurements; // Image measurement objects for error computation
  ...
  // Initialize body model geometry parameters and initial pose from
  // configuration file
  bool InitGeometry(const std::string& fname) {
    return mBodies[0].InitBodyShape(fname);
  };

  // Load initial state from a file
  bool LoadInitialState(const std::string& fname) {
    return mPoses[0].Initialize(fname);
  };

  // Load Body Pose Parameters from a file
  bool LoadPoseParameters(const std::string& fname) {
    return mPoses[0].InitParams(fname);
  };
...
public:
  ...
  // Load and process new observation data from image files for a given
  // time(frame).  Generates edge maps from the raw image files.
  virtual bool GetObservation(float timeval);

  // Returns body pose state - required by particle filter for initialization
  // only
  std::vector<float> InitialState() { return mPoses[0].Pose(); };

  // Calculates Log likelihood for a given set of body pose angles (and
  // translation - all are in the vector)
