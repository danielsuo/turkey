...
// The thread class constructor requires a threadable object to instantiate a
// thread object
class Runnable {
public:
  virtual ~Runnable(){};
...
// Abstract class which has to be implemented if a thread requires extra steps
// to stop
// Join() will call the Stop() method of a stoppable class before it waits for
// the termination of Run()
class Stoppable {
public:
  virtual ~Stoppable(){};
