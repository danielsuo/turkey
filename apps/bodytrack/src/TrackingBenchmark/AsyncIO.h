...
protected:
  ...
  void LoadSet(std::vector<std::string>& FGFiles, BinaryImageSet& FGimages,
               std::vector<std::string>& ImageFiles, ImageSet& images);

public:
  AsyncImageLoader()
      : mCondFull(mLock1), mCondEmpty(mLock2), mCurrentFrame(0), mNumCameras(0),
        mBufferSize(16), mFailed(false) {
