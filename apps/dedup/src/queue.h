...
static inline int ringbuffer_init(ringbuffer_t* buf, size_t size) {
  // NOTE: We have to allocate one extra element because one element will be
  // unusable (we need to distinguish between full and empty).
...
static inline void* ringbuffer_peek(ringbuffer_t* buf) {
  void* ptr;
