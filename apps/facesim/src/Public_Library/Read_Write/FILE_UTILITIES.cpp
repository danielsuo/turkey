...
std::string
Real_Path(const std::string& path) // TODO: Implement this for windows
{
...
int Compare_File_Times_Ignoring_Compression_Suffix(
  ...
  if (handle1 == INVALID_HANDLE_VALUE) {
    std::cerr << "Compare_File_Times: can't CreateFile " << filename1
              << std::endl;
    return 0;
  }
  ...
  if (handle2 == INVALID_HANDLE_VALUE) {
    std::cerr << "Compare_File_Times: can't CreateFile " << filename2
              << std::endl;
    return 0;
  }
  ...
  if (!GetFileTime(handle1, 0, &time1, 0) ||
      !GetFileTime(handle2, 0, &time2, 0)) {
...
bool Directory_Exists(const std::string& dirname) {
  ...
  return stat(dirname.c_str(), &fileInfo) == 0;
}

bool Create_Directory(const std::string& dirname, bool exit_on_fail) {
  if (!Directory_Exists(dirname)) {
    std::string command =
        STRING_UTILITIES::string_sprintf("mkdir -p %s", dirname.c_str());
...
std::istream* Safe_Open_Input(const std::string& filename, bool binary,
  ...
  if (File_Is_Compressed(filename) ||
    ...
    std::cerr << "Can't open " << filename
              << " (compression handling not enabled)\n";
    ...
    std::cerr << "Can't open " << filename << " for read "
...
std::ostream* Safe_Open_Output(const std::string& filename, bool binary,
  ...
  if (File_Is_Compressed(filename)) {
    ...
    std::cerr << "Refusing to write " << filename
              << " uncompressed when compressed version already exists\n";
    ...
    std::cerr << "Can't open " << filename << " for write "
              << (binary ? "(binary)" : "") << std::endl;
...
std::istream* Safe_Open_Input(const std::string& filename, bool binary,
  ...
  if (File_Exists(filename_compressed))
    return new ::igzstream(filename_compressed.c_str(), flags);

  if (!compressed) {
    std::string filename_uncompressed =
        compressed ? Strip_Compression_Suffix(filename) : filename;
  ...
  std::cerr << "Can't open " << filename_compressed << " for read "
            << (binary ? "(binary)" : "") << std::endl;
...
std::ostream* Safe_Open_Output(const std::string& filename, bool binary,
  ...
  if (compressed && !binary) {
    std::cerr << "Refusing to open compressed file " << filename
              << "in text mode\n";
  ...
  if (binary) {
    ...
    if (File_Writable(actual_filename))
      return new ::ogzstream(actual_filename.c_str(), flags);
  } else {
    actual_filename =
        compressed ? Strip_Compression_Suffix(filename) : filename;
  ...
  std::cerr << "Can't open " << actual_filename << " for write "
            << (binary ? "(binary)" : "") << std::endl;
...
int Compare_File_Times(const std::string& filename1,
  ...
  return Compare_File_Times_Ignoring_Compression_Suffix(Real_File(filename1),
                                                        Real_File(filename2));
}
//#####################################################################
