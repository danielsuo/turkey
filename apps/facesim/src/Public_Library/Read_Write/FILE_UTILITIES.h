//#####################################################################
// Copyright 2002-2004, Ronald Fedkiw, Eran Guendelman, Geoffrey Irving, Igor
// Neverov, Andrew Selle.
...
static const char* file_extensions[] = {
    "rgd", "rgd2d", "tri",   "phi",     "phi2d", "oct", "ply", "ply2d",
    "rgb", "tri2d", "curve", "curve2d", "tet",   "hex", 0};
//#####################################################################
...
int Compare_File_Times_Ignoring_Compression_Suffix(
...
//###################################################################
int Compare_File_Times(const std::string& filename1,
                       const std::string& filename2);
//###################################################################
// Platform Non-Specific Function Definitions
//###################################################################
inline std::string
Get_File_Extension_Ignoring_Compression_Suffix(const std::string& filename) {
  std::string::size_type lastperiod = filename.rfind('.'),
                         lastslash = filename.rfind('/');

  if (lastperiod != std::string::npos &&
      (lastslash == std::string::npos || lastperiod > lastslash))
...
inline std::string
Get_Basename_Ignoring_Compression_Suffix(const std::string& filename) {
  std::string::size_type lastperiod = filename.rfind(".");
...
inline bool File_Extension_Matches_Ignoring_Compression_Suffix(
    const std::string& filename, const std::string& ext,
    const bool case_sensitive = true) {
  return !STRING_UTILITIES::Compare_Strings(
      Get_File_Extension_Ignoring_Compression_Suffix(filename), ext,
      case_sensitive);
}
...
inline bool
File_Exists_Ignoring_Compression_Suffix(const std::string& filename) {
  struct stat fileInfo;
  return stat(filename.c_str(), &fileInfo) == 0;
}

inline bool File_Exists(const std::string& filename) {
  return File_Exists_Ignoring_Compression_Suffix(filename) ||
         (!File_Is_Compressed(filename) &&
          File_Exists_Ignoring_Compression_Suffix(filename + ".gz"));
}

inline bool
File_Writable_Ignoring_Compression_Suffix(const std::string& filename) {
  struct stat fileInfo;
  return stat(filename.c_str(), &fileInfo) == 0;
}

inline bool File_Writable(const std::string& filename) {
  return File_Writable_Ignoring_Compression_Suffix(filename) ||
         (!File_Is_Compressed(filename) &&
          File_Writable_Ignoring_Compression_Suffix(filename + ".gz"));
}

inline std::string Strip_Compression_Suffix(const std::string& filename) {
  if (File_Is_Compressed(filename))
    return Get_Basename_Ignoring_Compression_Suffix(filename);
  else
    return filename;
}

inline std::string Real_File(const std::string& filename) {
  if (File_Exists_Ignoring_Compression_Suffix(filename))
    return filename;
  else if (!File_Is_Compressed(filename) &&
           File_Exists_Ignoring_Compression_Suffix(filename + ".gz"))
    return filename + ".gz";
  else
    return "";
...
inline bool File_Extension_Matches(const std::string& filename,
                                   const std::string& ext,
                                   const bool case_sensitive = true) {
  return File_Extension_Matches_Ignoring_Compression_Suffix(
      Strip_Compression_Suffix(filename), ext, case_sensitive);
}

inline FILE_TYPE
Get_File_Type_Ignoring_Compression_Suffix(const std::string& filename) {
  std::string ext = Get_File_Extension_Ignoring_Compression_Suffix(filename);

  for (int i = 0; file_extensions[i]; i++)
...
inline FILE_TYPE Get_File_Type(const std::string& filename) {
  return Get_File_Type_Ignoring_Compression_Suffix(
      Strip_Compression_Suffix(filename));
}

inline bool
File_Type_Matches_Ignoring_Compression_Suffix(const std::string& filename,
                                              FILE_TYPE type) {
  return File_Extension_Matches_Ignoring_Compression_Suffix(
      filename, file_extensions[(int)type]);
}

inline bool File_Type_Matches(const std::string& filename, FILE_TYPE type) {
  return File_Type_Matches_Ignoring_Compression_Suffix(
      Strip_Compression_Suffix(filename), type);
}
...
inline std::string Get_File_Extension(const std::string& filename) {
  return Get_File_Extension_Ignoring_Compression_Suffix(
      Strip_Compression_Suffix(filename));
}

inline std::string Get_Basename(const std::string& filename) {
  return Get_Basename_Ignoring_Compression_Suffix(
      Strip_Compression_Suffix(filename));
}
...
inline std::string Find_First_Nonexistent_File_In_Sequence(
    std::string filename_base, std::string filename_suffix,
    const int id_start = 0, int* id_result = 0) {
  int i = id_start;

  while (File_Exists((filename_base + Number_To_String(i) + filename_suffix)))
    ++i;
...
inline std::string
Find_First_Nonexistent_Directory_In_Sequence(std::string directory_name_base,
                                             const int id_start = 0,
                                             int* id_final = 0) {
  int id = id_start;

  while (Directory_Exists((directory_name_base + Number_To_String(id))))
    ++id;
...
inline std::string
Make_First_Nonexistent_Directory_In_Sequence(std::string directory_name_base,
                                             const int id_start = 0,
                                             int* id_final = 0) {
  std::string output_directory = Find_First_Nonexistent_Directory_In_Sequence(
      directory_name_base, id_start, id_final);

  if (!Create_Directory(output_directory, false)) {
    std::cerr << "Can't create directory " << output_directory << std::endl;
    exit(1);
  } else
    std::cout << "Created directory " << output_directory << std::endl;
...
inline std::string Get_Frame_Filename(const std::string& filename, int frame) {
  return Is_Animated(filename)
             ? STRING_UTILITIES::string_sprintf(filename.c_str(), frame)
             : filename;
}
