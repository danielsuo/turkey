...
public:
  ...
  THREAD_POOL_SINGLETON_DATA() : queue(65536) {}
};
...
void* Worker(void* pointer) {
  ...
  while (1) {
    ...
    while (data->queue.Empty()) {
      ...
      if (data->working_threads == 0)
        data->workers_done_condition.Signal();
...
THREAD_POOL::THREAD_POOL() {
  ...
  if (threads_environment)
    number_of_threads = atoi(threads_environment);
  else
    number_of_threads = 2;
...
// Function Add_Task
//#####################################################################
void THREAD_POOL::Add_Task(CALLBACK callback_input, void* data_input) {
  data->lock.Lock();

  if (data->waiting_threads)
    data->new_work_condition.Signal();
...
//#####################################################################
void THREAD_POOL::Add_TaskGrid(CALLBACK callback_input, int numTasks) {
  // not supported
  exit(-1);
}
//#####################################################################
...
void THREAD_POOL::Wait_For_Completion() {
  ...
  while (!data->queue.Empty() || data->working_threads != 0)
    data->workers_done_condition.Wait(data->lock);
