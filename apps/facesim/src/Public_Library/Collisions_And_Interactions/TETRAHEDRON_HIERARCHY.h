...
template <class T> class TETRAHEDRON_HIERARCHY : public BOX_HIERARCHY<T> {
public:
  ...
  TETRAHEDRON_HIERARCHY(TETRAHEDRON_MESH& tetrahedron_mesh_input,
                        SOLIDS_PARTICLE<T, VECTOR_3D<T>>& particles_input,
                        const bool update_boxes = true)
      : tetrahedron_mesh(tetrahedron_mesh_input), particles(particles_input),
        tetrahedron_list(0) {
  ...
  TETRAHEDRON_HIERARCHY(TETRAHEDRON_MESH& tetrahedron_mesh_input,
                        SOLIDS_PARTICLE<T, VECTOR_3D<T>>& particles_input,
                        LIST_ARRAY<TETRAHEDRON<T>>& tetrahedron_list_input,
                        const bool update_boxes = true)
      : tetrahedron_mesh(tetrahedron_mesh_input), particles(particles_input),
        tetrahedron_list(&tetrahedron_list_input) {
    if (tetrahedron_mesh.tetrahedrons.m) {
      Initialize_Hierarchy_Using_KD_Tree();
  ...
  void Update_Boxes(const ARRAY<VECTOR_3D<T>>& X,
                    const T extra_thickness =
                        0) // use X instead of the current particle positions
  {
    Update_Leaf_Boxes(X, extra_thickness);
    Update_Nonleaf_Boxes();
  }

  void Update_Boxes(const ARRAY<VECTOR_3D<T>>& start_X,
                    const ARRAY<VECTOR_3D<T>>& end_X,
                    const T extra_thickness =
                        0) // bound tetrahedrons moving from start_X to end_X
  {
    Update_Leaf_Boxes(start_X, end_X, extra_thickness);
    Update_Nonleaf_Boxes();
  }

  void Update_Boxes(const RIGID_BODY_STATE_3D<T>& start_state,
                    const RIGID_BODY_STATE_3D<T>& end_state,
                    const T extra_thickness = 0) // for a moving rigid body
  {
  ...
  void Update_Leaf_Boxes(const T extra_thickness = 0) {
    ...
    if (extra_thickness)
      this->Thicken_Leaf_Boxes(extra_thickness);
  }

  void
  Update_Leaf_Boxes(const ARRAY<VECTOR_3D<T>>& X,
                    const T extra_thickness =
                        0) // use X instead of the current particle positions
  {
  ...
  void
  Update_Leaf_Boxes(const ARRAY<VECTOR_3D<T>>& start_X,
                    const ARRAY<VECTOR_3D<T>>& end_X,
                    const T extra_thickness =
                        0) // bound tetrahedrons moving from start_X to end_X
  {
  ...
  void Update_Leaf_Boxes(const RIGID_BODY_STATE_3D<T>& start_state,
                         const RIGID_BODY_STATE_3D<T>& end_state,
                         const T extra_thickness = 0) // for a moving rigid body
  {
  ...
  void Calculate_Bounding_Boxes(ARRAY<BOX_3D<T>>& bounding_boxes,
                                const ARRAY<VECTOR_3D<T>>& X);
  void Calculate_Bounding_Boxes(ARRAY<BOX_3D<T>>& bounding_boxes,
                                const ARRAY<VECTOR_3D<T>>& start_X,
                                const ARRAY<VECTOR_3D<T>>& end_X);
  void Calculate_Bounding_Boxes(ARRAY<BOX_3D<T>>& bounding_boxes,
                                const RIGID_BODY_STATE_3D<T>& start_state,
                                const RIGID_BODY_STATE_3D<T>& end_state);
  void Calculate_Bounding_Box_Radii(const ARRAY<BOX_3D<T>>& bounding_boxes,
                                    ARRAY<T>& radius);
  //#####################################################################
};
}
