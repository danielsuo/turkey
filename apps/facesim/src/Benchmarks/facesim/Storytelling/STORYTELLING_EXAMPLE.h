...
public:
  ...
  STORYTELLING_EXAMPLE(std::string data_directory = ".",
                       std::string output_directory = ".") {
    this->output_directory = output_directory;
    control_directory =
        data_directory + "/Face_Data/Motion_Data/Storytelling_Controls";
    ...
    solids_parameters.perform_collision_body_collisions =
        use_rigid_body_collision || use_self_collision;
  ...
  void Initialize_Bodies() {
    ...
    DEFORMABLE_OBJECT_3D<T>& deformable_object =
        solids_parameters.deformable_body_parameters.list(1);
    input_activation_controls = new ACTIVATION_CONTROL_SET<T>;

    for (int i = 1; i <= number_of_muscles; i++)
      input_activation_controls->Add_Activation();

    input_control_parameters.list.Append_Element(input_activation_controls);
    input_attachment_frame_controls = new ATTACHMENT_FRAME_CONTROL_SET<T>(
        deformable_object.particles.X.array, attached_nodes,
        jaw_attachment_index);
    input_attachment_frame_controls
        ->template Initialize_Jaw_Joint_From_File<RW>(input_directory +
                                                      "/jaw_joint_parameters");
    input_control_parameters.list.Append_Element(
        input_attachment_frame_controls);
  }
  //#####################################################################
  ...
  void Set_External_Controls(const T time) {
    ...
    T interpolation_fraction =
        fractional_control_frame - (T)integer_control_frame;
    std::string last_control_filename =
        control_directory + "/storytelling_controls." +
        STRING_UTILITIES::string_sprintf("%d", integer_control_frame);
    std::string next_control_filename =
        control_directory + "/storytelling_controls." +
        STRING_UTILITIES::string_sprintf("%d", integer_control_frame + 1);
    FILE_UTILITIES::Read_From_File<RW>(last_control_filename,
                                       input_control_parameters);
    input_control_parameters.Save_Controls();
    FILE_UTILITIES::Read_From_File<RW>(next_control_filename,
                                       input_control_parameters);
