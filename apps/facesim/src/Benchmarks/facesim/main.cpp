...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
  ...
  parse_args.Add_String_Argument("-input_dir", ".");
  parse_args.Add_String_Argument("-output_dir", ".");
  parse_args.Add_Option_Argument("-timing");
  parse_args.Parse(argc, argv);

  STORYTELLING_EXAMPLE<float, float> example(
      parse_args.Get_String_Value("-input_dir"),
      parse_args.Get_String_Value("-output_dir"));
  ...
  if (parse_args.Is_Value_Set("-threads")) {
    ...
    sprintf(tmp_buf, "PHYSBAM_THREADS=%d",
  ...
  if (PHYSBAM_THREADED_RUN == false &&
      parse_args.Get_Integer_Value("-threads") > 1) {
    printf(
        "Error: Number of threads cannot be greater than 1 for serial runs\n");
    exit(1);
  }

  THREAD_DIVISION_PARAMETERS<float>& parameters =
      *THREAD_DIVISION_PARAMETERS<float>::Singleton();
