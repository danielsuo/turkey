...
#define ERROR                                                                  \
  {                                                                            \
    printf("Error in TaskQ: This function should not have been called at  %s " \
           ": %d\n",                                                           \
           __FILE__, __LINE__);                                                \
    exit(0);                                                                   \
  }
...
#define DEBUG_ANNOUNCE                                                         \
  {                                                                            \
    printf("\n\n>>>>>>>>>>>>>>>>>>>>>>   Running the DEBUG version of the "    \
           "TaskQ   <<<<<<<<<<<<<<<<<<<\n\n\n");                               \
  }
#else
...
static inline void copyTask(void* taskDest[NUM_FIELDS],
                            void* taskSrc[NUM_FIELDS]) {
  int i;
...
static inline void copyArgs1(void* task[NUM_FIELDS], TaskQTask1 taskFunction,
                             void* arg1) {
  ...
  IF_DEBUG({
    ...
    for (i = 2; i < NUM_FIELDS; i++)
      task[i] = UNDEFINED_VALUE;
  });
}

static inline void copyArgs2(void* task[NUM_FIELDS], TaskQTask2 taskFunction,
                             void* arg1, void* arg2) {
...
static inline void copyArgs3(void* task[NUM_FIELDS], TaskQTask3 taskFunction,
...
typedef void (*AssignTasksFn)(TaskQTask3 taskFn, int numDimensions, int queueNo,
                              long min[MAX_DIMENSION], long max[MAX_DIMENSION],
                              long step[MAX_DIMENSION]);

void standardize(int n, long min1[MAX_DIMENSION], long max1[MAX_DIMENSION],
                 long step1[MAX_DIMENSION], long min2[MAX_DIMENSION],
                 long max2[MAX_DIMENSION], long step2[MAX_DIMENSION]) {
  long k;
...
static inline void assignTasksStandardized(AssignTasksFn fn, TaskQTask3 taskFn,
                                           int numDimensions, int queueNo,
                                           long min[MAX_DIMENSION],
...
static void enqueueGridRec(AssignTasksFn fn, TaskQTask3 taskFn,
                           int numDimensions, int startQueue, int totalQueues,
                           int currentDimension, long min[MAX_DIMENSION],
                           long max[MAX_DIMENSION], long step[MAX_DIMENSION]) {
  if (totalQueues == 1) {
    assignTasksStandardized(fn, taskFn, numDimensions, startQueue, min, max,
                            step);
  } else {
    long j, k;

    for (j = 0, k = currentDimension; j < numDimensions;
         j++, k = (k + 1) % numDimensions)
      if (max[k] - min[k] > 1)
        break;
    ...
    if (max[k] - min[k] == 1) {
      ...
      for (j = 1; j < totalQueues; j++)
        assignTasksStandardized(fn, taskFn, numDimensions, startQueue + j, min,
                                max, step);
    } else // Split it into two halfs and give half to each
    {
      ...
      enqueueGridRec(fn, taskFn, numDimensions, startQueue, half, next, min,
                     alt, step);
      ...
      enqueueGridRec(fn, taskFn, numDimensions, startQueue + half,
                     totalQueues - half, next, alt, max, step);
    }
  }
}
...
static inline void enqueueGridHelper(AssignTasksFn assignFunction,
                                     TaskQTask3 taskFunction, int numDimensions,
                                     int numTaskQs,
                                     long dimensionSize[MAX_DIMENSION],
  ...
  enqueueGridRec(assignFunction, taskFunction, numDimensions, 0, numTaskQs, 0,
                 min, max, tileSize);
}
