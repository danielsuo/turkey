...
static void x264_lowres_context_init(x264_t* h, x264_mb_analysis_t* a) {
  ...
  h->mb.i_me_method =
      X264_MIN(X264_ME_HEX, h->param.analyse.i_me_method); // maybe dia?
  h->mb.i_subpel_refine =
      4; // 3 should be enough, but not tweaking for speed now
  h->mb.b_chroma_me = 0;
}
...
static int x264_slicetype_mb_cost(x264_t* h, x264_mb_analysis_t* a,
  ...
  const int i_bipred_weight =
      h->param.analyse.b_weighted_bipred ? 64 - (dist_scale_factor >> 2) : 32;
  int16_t(*fenc_mvs[2])[2] = {&frames[b]->lowres_mvs[0][b - p0 - 1][i_mb_xy],
  ...
  h->mc.copy[PIXEL_8x8](h->mb.pic.p_fenc[0], FENC_STRIDE,
                        &fenc->lowres[0][i_pel_offset], i_stride, 8);
...
#define TRY_BIDIR(mv0, mv1, penalty)                                           \
  {                                                                            \
    ...
    h->mc.avg[PIXEL_8x8](pix1, 16, src1, stride1, src2, stride2,               \
                         i_bipred_weight);                                     \
    i_cost = penalty +                                                         \
             h->pixf.mbcmp[PIXEL_8x8](m[0].p_fenc[0], FENC_STRIDE, pix1, 16);  \
    if (i_bcost > i_cost)                                                      \
      i_bcost = i_cost;                                                        \
  }
  ...
  if (!b_bidir) {
    ...
    if (!fenc->b_intra_calculated) {
      ...
      if (h->pixf.intra_satd_x3_8x8c && h->pixf.mbcmp[0] == h->pixf.satd[0]) {
        ...
        satds[I_PRED_CHROMA_P] = h->pixf.satd[PIXEL_8x8](
            pix, FDEC_STRIDE, h->mb.pic.p_fenc[0], FENC_STRIDE);
      } else {
        for (i = 0; i < 4; i++) {
          h->predict_8x8c[i](pix);
          satds[i] = h->pixf.mbcmp[PIXEL_8x8](pix, FDEC_STRIDE,
      ...
      for (i = 3; i < 9; i++) {
        ...
        satd = h->pixf.mbcmp[PIXEL_8x8](pix, FDEC_STRIDE, h->mb.pic.p_fenc[0],
                                        FENC_STRIDE);
    ...
    if (b_intra)
      i_bcost = i_icost;
    if (i_mb_x > 0 && i_mb_x < h->sps->i_mb_width - 1 && i_mb_y > 0 &&
        i_mb_y < h->sps->i_mb_height - 1) {
...
static int x264_slicetype_frame_cost(x264_t* h, x264_mb_analysis_t* a,
                                     x264_frame_t** frames, int p0, int p1,
                                     int b, int b_intra_penalty) {
  int i_score = 0;
  ...
  if (frames[b]->i_cost_est[b - p0][p1 - b] >= 0 &&
      (!h->param.rc.i_vbv_buffer_size ||
       frames[b]->i_row_satds[b - p0][p1 - b][0] != -1)) {
    i_score = frames[b]->i_cost_est[b - p0][p1 - b];
  } else {
    int dist_scale_factor = 128;
    int* row_satd = frames[b]->i_row_satds[b - p0][p1 - b];

    /* For each list, check to see whether we have lowres motion-searched this
     * reference frame before. */
    do_search[0] =
        b != p0 && frames[b]->lowres_mvs[0][b - p0 - 1][0][0] == 0x7FFF;
    do_search[1] =
        b != p1 && frames[b]->lowres_mvs[1][p1 - b - 1][0][0] == 0x7FFF;
    if (do_search[0])
    ...
    if (h->sps->i_mb_width <= 2 || h->sps->i_mb_height <= 2) {
      for (h->mb.i_mb_y = h->sps->i_mb_height - 1; h->mb.i_mb_y >= 0;
           h->mb.i_mb_y--)
        for (h->mb.i_mb_x = h->sps->i_mb_width - 1; h->mb.i_mb_x >= 0;
             h->mb.i_mb_x--)
          i_score += x264_slicetype_mb_cost(h, a, frames, p0, p1, b,
                                            dist_scale_factor, do_search);
    }
     ...
     * whole frame's score, but are needed for a spatial distribution. */
      ...
      for (h->mb.i_mb_y = h->sps->i_mb_height - 1; h->mb.i_mb_y >= 0;
        ...
        for (h->mb.i_mb_x = h->sps->i_mb_width - 1; h->mb.i_mb_x >= 0;
             h->mb.i_mb_x--) {
          int i_mb_cost = x264_slicetype_mb_cost(h, a, frames, p0, p1, b,
                                                 dist_scale_factor, do_search);
          int i_mb_cost_aq = i_mb_cost;
          if (h->param.rc.i_aq_mode)
            i_mb_cost_aq =
                (i_mb_cost_aq *
                     frames[b]->i_inv_qscale_factor[h->mb.i_mb_x +
                                                    h->mb.i_mb_y *
                                                        h->mb.i_mb_stride] +
                 128) >>
                8;
          ...
          if (h->mb.i_mb_y > 0 && h->mb.i_mb_y < h->sps->i_mb_height - 1 &&
            ...
            /* Don't use AQ-weighted costs for slicetype decision, only for
             * ratecontrol. */
            i_score += i_mb_cost;
      ...
      for (h->mb.i_mb_y = h->sps->i_mb_height - 2; h->mb.i_mb_y > 0;
           h->mb.i_mb_y--)
        for (h->mb.i_mb_x = h->sps->i_mb_width - 2; h->mb.i_mb_x > 0;
             h->mb.i_mb_x--) {
          int i_mb_cost = x264_slicetype_mb_cost(h, a, frames, p0, p1, b,
                                                 dist_scale_factor, do_search);
          int i_mb_cost_aq = i_mb_cost;
...
static int x264_slicetype_path_cost(x264_t* h, x264_mb_analysis_t* a,
                                    x264_frame_t** frames, char* path,
                                    int threshold) {
  int loc = 1;
  int cost = 0;
  int cur_p = 0;
  ...
  while (path[loc]) {
    ...
    /* Early terminate if the cost we have found is larger than the best path
     * cost so far */
    if (cost > threshold)
      break;
...
static void x264_slicetype_path(x264_t* h, x264_mb_analysis_t* a,
                                x264_frame_t** frames, int length,
                                int max_bframes, int buffer_size,
                                char (*best_paths)[MAX_LENGTH]) {
  ...
  for (suffix_size = 0; suffix_size < num_paths; suffix_size++) {
    memcpy(paths[suffix_size], best_paths[length - (suffix_size + 1)],
           length - (suffix_size + 1));
...
static int x264_slicetype_path_search(x264_t* h, x264_mb_analysis_t* a,
                                      x264_frame_t** frames, int length,
                                      int bframes, int buffer) {
...
static int scenecut(x264_t* h, x264_frame_t* frame, int pdist) {
  ...
  float f_thresh_min =
      f_thresh_max * h->param.i_keyint_min / (h->param.i_keyint_max * 4);
  int res;
  ...
  if (res) {
    ...
    int pmb = NUM_MBS - imb;
    x264_log(h, X264_LOG_DEBUG, "scene cut at %d Icost:%d Pcost:%d ratio:%.4f "
                                "bias:%.4f gop:%d (imb:%d pmb:%d)\n",
...
static void x264_slicetype_analyse(x264_t* h) {
  ...
  x264_frame_t* frames[X264_BFRAME_MAX * 4 + 3] = {
      NULL,
  };
  int num_frames;
  int keyint_limit;
  ...
  keyint_limit =
      h->param.i_keyint_max - frames[0]->i_frame + h->frames.i_last_idr - 1;
  ...
  idr_frame_type =
      frames[1]->i_frame - h->frames.i_last_idr >= h->param.i_keyint_min
          ? X264_TYPE_IDR
          : X264_TYPE_I;
  ...
  if (h->param.i_bframe_adaptive == X264_B_ADAPT_TRELLIS) {
    ...
    num_bframes = x264_slicetype_path_search(
        h, &a, frames, num_frames, max_bframes, num_frames - max_bframes);
...
#define P_SENS_BIAS (50 - h->param.i_bframe_bias)
    ...
    for (j = 2; j <= X264_MIN(h->param.i_bframe, num_frames - 1); j++) {
      int pthresh =
          X264_MAX(INTER_THRESH - P_SENS_BIAS * (j - 1), INTER_THRESH / 10);
      int pcost = x264_slicetype_frame_cost(h, &a, frames, 0, j + 1, j + 1, 1);

      if (pcost > pthresh * i_mb_count ||
          frames[j + 1]->i_intra_mbs[j + 1] > i_mb_count / 3) {
        frames[j]->i_type = X264_TYPE_P;
        break;
      } else
        frames[j]->i_type = X264_TYPE_B;
  ...
  for (bframes = 0;; bframes++) {
    ...
    if (frm->i_frame - h->frames.i_last_idr >= h->param.i_keyint_max) {
      ...
      if (frm->i_type != X264_TYPE_IDR)
        x264_log(h, X264_LOG_WARNING, "specified frame type (%d) is not "
...
int x264_rc_analyse_slice(x264_t* h) {
  x264_mb_analysis_t a;
  x264_frame_t* frames[X264_BFRAME_MAX * 4 + 2] = {
      NULL,
  };
  int p0 = 0, p1, b;
  int cost;
  ...
  if (IS_X264_TYPE_I(h->fenc->i_type)) {
    ...
    p1 = 0;
    while (h->frames.current[p1] &&
           IS_X264_TYPE_B(h->frames.current[p1]->i_type))
      p1++;
    p1++;
    b = p1;
  } else // B
  ...
  h->fdec->i_satd = cost;
  memcpy(h->fdec->i_row_satd, h->fenc->i_row_satd,
         h->sps->i_mb_height * sizeof(int));
  return cost;
}
