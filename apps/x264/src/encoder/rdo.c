...
#define x264_cabac_encode_decision_noup(c, x, v)                               \
  x264_cabac_size_decision_noup(c, x, v)
#define x264_cabac_encode_terminal(c) x264_cabac_size_decision_noup(c, 276, 0)
#define x264_cabac_encode_bypass(c, v) ((c)->f8_bits_encoded += 256)
#define x264_cabac_encode_ue_bypass(c, e, v)                                   \
  ((c)->f8_bits_encoded += (bs_size_ue_big(v + (1 << e) - 1) - e) << 8)
#define x264_cabac_encode_flush(h, c)
#define x264_macroblock_write_cabac static x264_macroblock_size_cabac
#include "cabac.c"

#define COPY_CABAC                                                             \
  h->mc.memcpy_aligned(&cabac_tmp.f8_bits_encoded, &h->cabac.f8_bits_encoded,  \
                       sizeof(x264_cabac_t) -                                  \
                           offsetof(x264_cabac_t, f8_bits_encoded))
...
/* The difference between SATD and SA8D scores are both used to avoid bias from
 * the DCT size.  Using SATD */
/* only, for example, results in overusage of 8x8dct, while the opposite occurs
 * when using SA8D. */

/* FIXME:  Is there a better metric than averaged SATD/SA8D difference for
 * complexity difference? */
/* Hadamard transform is recursive, so a SATD+SA8D can be done faster by taking
...
static inline int ssd_plane(x264_t* h, int size, int p, int x, int y) {
  ...
  if (p == 0 && h->mb.i_psy_rd) {
    /* If the plane is smaller than 8x8, we can't do an SA8D; this probably
     * isn't a big problem. */
    if (size <= PIXEL_8x8) {
      ...
      satd = abs(h->pixf.satd[size](fdec, FDEC_STRIDE, zero, 0) - dc -
                 sum_satd(h, size, x, y));
    }
...
static inline int ssd_mb(x264_t* h) {
  return ssd_plane(h, PIXEL_16x16, 0, 0, 0) + ssd_plane(h, PIXEL_8x8, 1, 0, 0) +
...
/* partition RD functions use 8 bits more precision to avoid large rounding
 * errors at low QPs */

static uint64_t x264_rd_cost_subpart(x264_t* h, int i_lambda2, int i4,
                                     int i_pixel) {
...
static uint64_t x264_rd_cost_i8x8(x264_t* h, int i_lambda2, int i8,
                                  int i_mode) {
  uint64_t i_ssd, i_bits;
...
static uint64_t x264_rd_cost_i4x4(x264_t* h, int i_lambda2, int i4,
                                  int i_mode) {
  uint64_t i_ssd, i_bits;
...
/* precalculate the cost of coding various combinations of bits in a single
 * context */
...
static const int lambda2_tab[2][52] = {
    ...
    {46,      58,      73,      92,     117,     147,     185,     233,
     294,     370,     466,     587,    740,     932,     1174,    1480,
     1864,    2349,    2959,    3728,   4697,    5918,    7457,    9395,
     11837,   14914,   18790,   23674,  29828,   37581,   47349,   59656,
     75163,   94699,   119313,  150326, 189399,  238627,  300652,  378798,
     477255,  601304,  757596,  954511, 1202608, 1515192, 1909022, 2405217,
     3030384, 3818045, 4810435, 6060769},
    // intra lambda = .65 * .65 * 2**(qp/3. + 10 - LAMBDA_BITS)
    {27,      34,      43,      54,     68,     86,     108,     136,
     172,     216,     273,     343,    433,    545,    687,     865,
     1090,    1374,    1731,    2180,   2747,   3461,   4361,    5494,
     6922,    8721,    10988,   13844,  17442,  21976,  27688,   34885,
     43953,   55377,   69771,   87906,  110755, 139543, 175813,  221511,
     279087,  351627,  443023,  558174, 703255, 886046, 1116348, 1406511,
...
// save quantized coefs during rd, to avoid a duplicate trellis in the final
// encode
...
quant_trellis_cabac(x264_t* h, int16_t* dct, const uint16_t* quant_mf,
                    const int* unquant_mf, const int* coef_weight,
                    const uint8_t* zigzag, int i_ctxBlockCat, int i_lambda2,
                    int b_ac, int dc, int i_coefs, int idx) {
  ...
  // we don't need to keep all of those: (# of coefs) * (# of ctx) would be
  // enough,
  // but it takes more time to remove dead states than you gain in reduced
  // memory.
  struct {
    uint16_t abs_level;
    uint16_t next;
  ...
  for (i = i_coefs - 1; i >= b_ac; i--)
    if ((unsigned)(dct[zigzag[i]] *
                       (dc ? quant_mf[0] >> 1 : quant_mf[zigzag[i]]) +
                   f - 1) >= 2 * f)
      break;
  ...
  // coefs are processed in reverse order, because that's how the abs value is
  // coded.
  // last_coef and significant_coef flags are normally coded in forward order,
  // but
  // we have to reverse them to match the levels.
  // in 4x4 blocks, last_coef and significant_coef use a separate context for
  // each
  // position, so the order doesn't matter, and we don't even have to update
  // their contexts.
  // in 8x8 blocks, some positions share contexts, so we'll just have to hope
  // that
  ...
  if (i_coefs == 64) {
    const uint8_t* ctx_sig =
        &h->cabac
             .state[significant_coeff_flag_offset[b_interlaced][i_ctxBlockCat]];
    const uint8_t* ctx_last =
        &h->cabac.state[last_coeff_flag_offset[b_interlaced][i_ctxBlockCat]];
    for (i = 0; i < 63; i++) {
      cabac_state_sig[i] =
          ctx_sig[significant_coeff_flag_offset_8x8[b_interlaced][i]];
  ...
  memcpy(nodes_cur[0].cabac_state,
         &h->cabac.state[coeff_abs_level_m1_offset[i_ctxBlockCat]], 10);
  ...
  for (i = i_last_nnz; i >= b_ac; i--) {
    ...
    // skip 0s: this doesn't affect the output, but saves some unnecessary
    // computation.
    if (q == 0) {
      // no need to calculate ssd of 0s: it's the same in all nodes.
      // no need to modify level_tree for ctx=0: it starts with an infinite loop
      // of 0s.
      const uint32_t cost_sig0 =
          x264_cabac_size_decision_noup2(&cabac_state_sig[i], 0) *
              (uint64_t)i_lambda2 >>
...
#define SET_LEVEL(n, l)                                                        \
  ...
  i_levels_used++;
    ...
    // but it's only around .003 dB, and skipping them ~doubles the speed of
    // trellis.
    // could also try q-2: that sometimes helps, but also sometimes decimates
    // blocks
    // that are better left coded, especially at QP > 40.
    for (abs_level = q; abs_level >= q - 1; abs_level--) {
      int unquant_abs_level =
          (((dc ? unquant_mf[0] << 1 : unquant_mf[zigzag[i]]) * abs_level +
            128) >>
           8);
      int d = i_coef - unquant_abs_level;
      int64_t ssd;
      /* Psy trellis: bias in favor of higher AC coefficients in the
       * reconstructed frame. */
      if (h->mb.i_psy_trellis && i && !dc && i_ctxBlockCat != DCT_CHROMA_AC) {
        int orig_coef = (i_coefs == 64) ? h->mb.pic.fenc_dct8[idx][i]
                                        : h->mb.pic.fenc_dct4[idx][i];
        int predicted_coef = orig_coef - i_coef * signs[i];
        int psy_value = h->mb.i_psy_trellis *
                        abs(predicted_coef + unquant_abs_level * signs[i]);
        int psy_weight = (i_coefs == 64) ? x264_dct8_weight_tab[zigzag[i]]
                                         : x264_dct4_weight_tab[zigzag[i]];
        ...
        if (abs_level || node_ctx) {
          ...
          if (abs_level) {
            ...
            f8_bits += x264_cabac_size_decision2(
                &n.cabac_state[coeff_abs_level1_ctx[node_ctx]], i_prefix > 0);
            if (i_prefix > 0) {
          ...
          n.score +=
              (uint64_t)f8_bits * i_lambda2 >> (CABAC_SIZE_BITS - LAMBDA_BITS);
        }

        n.score += ssd;

        /* save the node if it's better than any existing node with the same
         * cabac ctx */
...
void x264_quant_dc_trellis(x264_t* h, int16_t* dct, int i_quant_cat, int i_qp,
                           int i_ctxBlockCat, int b_intra) {
  quant_trellis_cabac(h, (int16_t*)dct, h->quant4_mf[i_quant_cat][i_qp],
                      h->unquant4_mf[i_quant_cat][i_qp], NULL,
                      i_ctxBlockCat == DCT_CHROMA_DC
                          ? x264_zigzag_scan2
                          : x264_zigzag_scan4[h->mb.b_interlaced],
                      i_ctxBlockCat, lambda2_tab[b_intra][i_qp], 0, 1,
                      i_ctxBlockCat == DCT_CHROMA_DC ? 4 : 16, 0);
}
...
void x264_quant_4x4_trellis(x264_t* h, int16_t dct[4][4], int i_quant_cat,
  ...
  quant_trellis_cabac(h, (int16_t*)dct, h->quant4_mf[i_quant_cat][i_qp],
                      h->unquant4_mf[i_quant_cat][i_qp],
                      x264_dct4_weight2_zigzag[h->mb.b_interlaced],
                      x264_zigzag_scan4[h->mb.b_interlaced], i_ctxBlockCat,
                      lambda2_tab[b_intra][i_qp], b_ac, 0, 16, idx);
}

void x264_quant_8x8_trellis(x264_t* h, int16_t dct[8][8], int i_quant_cat,
                            int i_qp, int b_intra, int idx) {
  quant_trellis_cabac(h, (int16_t*)dct, h->quant8_mf[i_quant_cat][i_qp],
                      h->unquant8_mf[i_quant_cat][i_qp],
                      x264_dct8_weight2_zigzag[h->mb.b_interlaced],
                      x264_zigzag_scan8[h->mb.b_interlaced], DCT_LUMA_8x8,
                      lambda2_tab[b_intra][i_qp], 0, 0, 64, idx);
}
