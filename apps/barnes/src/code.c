#line 233 "/home/ubuntu/parsec/pkgs/libs/parmacs/inst/amd64-linux.gcc/m4/parmacs.pthreads.c.m4"
...
int main (int argc, string argv[])
{
  ...
  while ((c = getopt(argc, argv, "hn:i:")) != -1) {
    switch(c) {
      case 'h':
        Help();
        exit(-1);
...
int get_int_param(FILE *fp) {
  string result = get_param(fp);
  int ret = atoi(result);
  free(result);
  result = NULL;
  return ret;
...
double get_double_param(FILE *fp) {
  string result = get_param(fp);
  double ret = atof(result);
  free(result);
  result = NULL;
  return ret;
}
...
void startrun()
{
  ...
  if (infile[0] != '\n') {
    ...
    nbody = get_int_param(fp);
    if (nbody < 1) {
      error("startrun: absurd nbody\n");
    }
    seed = get_int_param(fp);
  }

  outfile = get_param(fp);
  dtime = get_double_param(fp);
  dthf = 0.5 * dtime;
  eps = get_double_param(fp);
  epssq = eps * eps;
  tol = get_double_param(fp);
  tolsq = tol * tol;
  fcells = get_double_param(fp);
  fleaves = get_double_param(fp);
  tstop = get_double_param(fp);
  dtout = get_double_param(fp);
