...
// Use to replace pthread barriers on machines that have pthreads but not
// pthread barriers

// Some comments on this barrier implementation and directions for possible
// improvements:
//
//   * This is a straightforward two-phase centralized barrier implementation
//   * It's main advantage is that it only requries the pthreads library (no
//   atomic instructions),
//     making it very portable and easy to maintain if libpthread is already
//     installed
//   * The only more advanced feature is the ability to spin (if enabled). This
//   should yield
//     significant performance improvements if the number of threads is less or
//     equal to the
//     number of processors
//   * There are many ways to improve this implementation, just check the
//   scientific literature.
//     Some suggestions are:
//       - Dynamically adapt maximum amount of spinning based on observed
//       runtime behavior
...
static inline void Not_Implemented(const char* function, const char* file,
                                   unsigned int line) {
  fprintf(stderr, "ERROR: Something in function %s in file %s, line %u is not "
                  "implemented.\n",
          function, file, line);
  exit(1);
}
...
int parsec_barrier_init(parsec_barrier_t* barrier,
                        const parsec_barrierattr_t* attr, unsigned count) {
  int rv;
...
int parsec_barrier_destroy(parsec_barrier_t* barrier) {
  ...
  // have returned an error, but we check anyway to catch any other unexpected
  // errors.
  if (barrier->n != 0)
    return EBUSY;
  return 0;
...
int parsec_barrierattr_getpshared(const parsec_barrierattr_t* attr,
                                  int* pshared) {
...
int parsec_barrier_wait(parsec_barrier_t* barrier) {
  ...
  if (rv != 0)
    return rv;
// First we must wait to be sure that all threads from a previous barrier use
// had
// the chance to leave (departure phase complete) before we can reuse the
// barrier
#ifndef ENABLE_SPIN_BARRIER
  // Standard method to block on a condition variable (with simple error
  // propagation)
  ...
  // We are guaranteed to be in an arrival phase, proceed with barrier
  // synchronization
...
#else
    // we use again an unsynchronized polling loop followed by synchronized
    // fall-back blocking
    if (barrier->is_arrival_phase) {
      pthread_mutex_unlock(&barrier->mutex);
      volatile spin_counter_t i = 0;
...
const int PRIME_NUMBER =
    31; // An arbitrarily chosen prime to mix up access patterns a little
const int NTHREADS = 4; // Number of threads to check barrier with
const int NBUGGERS = 3; // Maximum number of `bugger' threads to use
const int WORK_UNIT_WORKER = 2 * 131072; // Work unit for tester threads (must
                                         // be power of two due to overflow)
const int WORK_UNIT_BUGGER =
    WORK_UNIT_WORKER /
    128; // Work unit for bugger threads (must be power of two due to overflow)
volatile int
    terminate_bugger_threads; // flag to signal termination to bugger threads
...
// A simple background thread, designed to disturb the timing behavior of the
// worker threads
void* bugger_thread(void* arg) {
  ...
  // do something simple to occupy the processor and yield to cause unexpected
  // delays
...
void* counter_thread(void* arg) {
  ...
  // Increment a set of counters in parallel, pseudo-randomly pick which counter
  // to increment
  // All counter values are always the same after each step, unless there's a
  // race condition
  for (i = 0; i < WORK_UNIT_WORKER; i++) {
    unsigned int idx;
    test_counter_t temp;
